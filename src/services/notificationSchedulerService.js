...
class NotificationSchedulerService {
  ...
  async initialize() {
    ...
    try {
      ...
      // 🔴 FIX #3: INITIALIZE FCM PUSH NOTIFICATIONS
  ...
  async initializeFCM() {
    try {
      ...
      if (!Capacitor.isNativePlatform()) {
        if(import.meta.env.DEV)console.log('⚠️ FCM not available on web platform');
        return;
      }
      ...
      try {
        ...
        // 🔴 FIX #3: LISTEN FOR INCOMING PUSH NOTIFICATIONS
        PushNotifications.addListener('pushNotificationReceived', (notification) => {
          if(import.meta.env.DEV)console.log('📬 Push notification received:', notification);
          
          const title = notification.title || 'WellnessAI';
        ...
        PushNotifications.addListener('pushNotificationActionPerformed', (notification) => {
          if(import.meta.env.DEV)console.log('🔔 Push notification action:', notification);
          if (notification.notification.data?.screen) {
            window.location.hash = notification.notification.data.screen;
          }
        });
        
        if(import.meta.env.DEV)console.log('✅ FCM push notifications initialized');
      } catch (fcmError) {
        if(import.meta.env.DEV)console.warn('⚠️ PushNotifications plugin not available:', fcmError.message);
      }
    } catch (error) {
      if(import.meta.env.DEV)console.warn('⚠️ FCM initialization failed:', error);
    }
  }
