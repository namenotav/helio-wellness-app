 ...
 * Reads step data from Android Health Connect (system-level health database)
 * Works on Android 14+ (API 34+)
 * Reads from Google Fit, Samsung Health, Fitbit, and other fitness apps
 */
...
class HealthConnectService {
  ...
  async checkAvailability() {
    try {
      ...
      if (import.meta?.env?.DEV) console.log('📡 HealthConnect.isAvailable result:', availability);
      return availability;
    } catch (error) {
  ...
  async requestPermissions() {
    try {
      ...
      // Re-check permissions after user grants
      setTimeout(async () => {
        const perms = await HealthConnect.hasPermissions();
        this.hasPermissions = perms.hasAllPermissions;
