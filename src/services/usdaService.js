// USDA FoodData Central Service - 400,000+ official food database
// SECURITY: Load API key from environment to prevent exposure
...
class USDAService {
  ...
  async searchFoods(query, pageSize = 20) {
    ...
    try {
      const params = new URLSearchParams({
        query: query.trim(),
        pageSize: Math.min(pageSize, 50), // Limit to 50 for performance
        dataType: 'Branded,Foundation,SR Legacy', // Include branded, foundation, and legacy foods
  ...
  async getFoodDetails(fdcId) {
    try {
      ...
      if(import.meta.env.DEV) console.error('❌ USDA details error:', error);
      return {
        success: false,
  ...
  async getBatchFoods(fdcIds) {
    ...
    try {
      ...
      if(import.meta.env.DEV) console.error('❌ USDA batch error:', error);
      return {
        success: false,
