...
export default function ProgressModal({ isOpen, onClose, todaySteps = 0 }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // 🔥 FIX: Add 5s polling for real-time updates (was missing!)
  ...
  const loadProgressData = async () => {
    try {
      // 🎯 SINGLE SOURCE OF TRUTH: Use dataService (4-system architecture)
      const { default: authService } = await import('../services/authService.js');
      const userId = authService.getCurrentUser()?.uid;
      
      // 🎯 WORKOUTS: Read from localStorage (instant updates)
      const workoutLog = JSON.parse(localStorage.getItem('workoutHistory') || '[]');
      
      // 🎯 STEP HISTORY: Read from Android's CapacitorStorage (wellnessai_ prefix)
      const stepHistoryResult = await Preferences.get({ key: 'wellnessai_stepHistory' });
      const stepHistoryRaw = JSON.parse(stepHistoryResult.value || '[]');
      console.log('📈 Progress Modal loaded from Android Preferences:', stepHistoryRaw?.length, 'entries');
