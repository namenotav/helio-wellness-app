...
const chatSchema = Joi.object({
  message: Joi.string().min(1).max(5000).required(),  // Increased to 5000 for meal automation prompts
...
app.post('/api/v1/chat', async (req, res) => {
  try {
    ...
    const sanitizedMessage = String(message)
      .trim()
      .slice(0, 5000) // Limit to 5000 characters (matches chatSchema)
...
// Backward compatibility: Forward old /api/chat to /api/v1/chat handler
app.post('/api/chat', async (req, res, next) => {
  // Properly forward to v1 endpoint by calling the handler directly
  req.url = '/api/v1/chat';
